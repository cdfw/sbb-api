...
public interface CSUserLaborClassInputRepository extends CrudRepository<CSUserLaborClassInputEntity, Integer>{
	...
	@Modifying
	@Transactional
	@Query("UPDATE CSUserLaborClassInputEntity lc SET inputHours = :inputHours, feedback = :feedback, updtdDtm = current_timestamp, editedBy = :editedBy, lc.sttsId = 'A', lc.approverId=:editedBy where lc.userId = :userId and lc.regionId = :regionId and lc.positionId = :positionId and lc.taskId = :taskId")
    public void editAndApproveCsInput(
    		@Param("userId") int userId, 
    		@Param("regionId") int regionId, 
    		@Param("positionId")String positionId, 
    		@Param("taskId")String taskId,
	...
	@Transactional
	@Query("UPDATE CSUserLaborClassInputEntity lc SET  lc.sttsId = 'A', lc.updtdDtm = current_timestamp, lc.approverId=:approverId"
			+ " where lc.regionId = :regionId and lc.positionId = :positionId and lc.sttsId <> 'A'")
    public void approveAllInputs(
    		@Param("regionId") int regionId, 
    		@Param("positionId")String positionId, 
    		@Param("approverId")int approverId		
    		);
	...
	@Modifying
	@Transactional
	@Query("UPDATE CSUserLaborClassInputEntity lc SET  lc.sttsId = 'A', inputHours = :inputHours, feedback = :feedback, lc.updtdDtm = current_timestamp, lc.approverId=:approverId, editedBy = :approverId"
			+ " where lc.userId = :userId and lc.regionId = :regionId and lc.positionId = :positionId and lc.taskId = :taskId")
    public void updateAndValidateCsInput(
    		@Param("userId") int userId, 
    		@Param("regionId") int regionId, 
    		@Param("positionId")String positionId, 
    		@Param("taskId")String taskId,
	...
	@Query("SELECT new com.sbb.model.CSUserLaborClassInputEntityModel(lc.positionId, SUM(lc.inputHours) as hoursEntered) from CSUserLaborClassInputEntity lc "
			+ " where lc.regionId = :regionId and lc.sttsId ='A'"
			+ " group by lc.positionId")
	public List<CSUserLaborClassInputEntityModel> findValidatedHours(int regionId);
